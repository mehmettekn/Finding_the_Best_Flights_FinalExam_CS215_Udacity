...
def make_link(G, city1, city2, departure, arrival, cost, flight_num):
    if city1 not in G:
        G[city1] = {}
    ...
    if city2 not in G:
        G[city2] = {}
    if city1 not in G[city2]:
        G[city2][city1] = {}
    G[city1][city2].append((departure, arrival, cost, flight_num))
    G[city2][city1].append((departure, arrival, cost, flight_num))
    return G

def create_graph(data):
    G = {}
    for flight_num, city1, city2, departure,
        arrival, cost in data:
            make_link(G, city1, city2, departure,
                      arrival, cost, flight_num)
    return G

            
def find_best_flights(flights, origin, destination):
    G = create_graph(all_flights)
    cost_so_far = priority_dict([i])
    cost = {i:0}
    paths = {i:[i]}
    
    while len(cost_so_far) != 0:
        w = cost_so_far.pop_smallest()
        for x in G[w]:
            new_cost = min(cost[w], G[w][x][2])
            if x not in cost or new_cost > cost[x]:
                cost[x] = new_score
                paths[x] = paths[w] + [(x, G[w][x][3])]
    
    if destination not in paths: return None
    return paths[destination][1:][1]
